...
const AdminSettings = () => {
  // CORRECTION ICI : Remplacement de disableGlobal par disableToggle (et ajout de disabling si tu veux mettre un spinner)
  const { enabled, disabled, toggle, disableToggle, disabling } = useSwitch();
          ...
          <div className="bg-zinc-950/40 border border-white/[0.05] backdrop-blur-md rounded-2xl p-6 shadow-xl space-y-6">
            ...
            <div className="space-y-6 divide-y divide-white/[0.05]">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-2">
                ...
                {/* BOUTON TOGGLE DISABLE CORRIGÉ */}
                <button
                  disabled={disabling} // Évite les doubles clics pendant la mutation GraphQL
                  onClick={disableToggle} // APPEL DE LA BONNE FONCTION DU CONTEXT
          ...
          {disabled && (
            <div className="flex items-start gap-3 bg-red-950/20 border border-red-500/10 rounded-xl p-4 text-xs text-red-300">
              ...
              <p className="leading-relaxed">
                <strong>Mode Sécurité Activé :</strong> Le modal d'événement est actuellement forcé à l'état "éteint" au niveau de la base de données. Pour pouvoir modifier à nouveau son affichage standard dans vos cookies, vous devez d'abord désactiver le commutateur de blocage ci-dessus.
              </p>
            </div>
          )}
